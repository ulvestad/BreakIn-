...
/**
 * Reduces the balls velocity when hit
 */
public class SlowBrick extends Brick {

    private volatile boolean isSpeedChanged;

    private Timer timer;

    private Ball ball;

    public SlowBrick(GameState gameState){
        super(gameState);

        brickScore = Constants.BRICK_SCORE_SLOW;
        brickTexture = new Texture(Constants.TEXTURE_BRICK_SLOW);
    ...
    public void onBreak(Ball ball) {
        ...
        if(!isSpeedChanged){
            ...
            timer = new Timer();
            timer.schedule(new ResetSpeedTask(), Constants.SLOW_BRICK_TIME);
        }
